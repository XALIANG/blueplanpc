const map = {},
  pre = 'data.';
const layoutType = 'SET_LAYOUT_SETTING';
const FormType = 'SET_FORM_SETTING';
...
function select(data, _bind_) {
  ...
  let bind = fnBind(data, _bind_);
  return `<a-form-model-item size='${data.size}' label='${data.name}'><a-select ${bind ? 'v-model="' + pre + data.value + '"' : ''} style='width: ${data.width}' id='${data.key}' ${data.disabled ? 'disabled' : ''} placeholder='${data.placeholder}' value=''>${item(
    data.option
...
class encoder {
  ...
  build(data, type) {
    ...
    if (type === layoutType) {
      return {
        template: `<a-layout>${fnBuild(data, local, fnBind, fnBuild)}</a-layout>`,
        data() {
          return {
            data: local
          };
