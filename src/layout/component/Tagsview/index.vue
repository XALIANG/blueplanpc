...
<div id="tags-view-container">
    <ul class="tags-box">
        <li v-for="(item,index) in tagList" :key="index" :class="{'active':isAcitive(item.path)}">
            <router-link :to="item.path">
                <span> {{item.title}}</span>
            </router-link>
            <span :class="{'icon-active':hoemApp(item)}" @click="closeTags(index)">
...
export default {
    ...
    methods: {
        ...
        closeTags(idx) {
            ...
            const item = this.tagList[idx] ? this.tagList[idx] : this.tagList[idx - 1];
            if (item) {
                deleteItem.path === this.$route.fullPath && this.$router.push(item.path);
            } else {
                this.$router.push('/');
            }
