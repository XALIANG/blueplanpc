...
import { userSetCode, userOutCode } from '../../apis/user';
export default {
  name: 'WebSocket',
  props: ['params'],
  data() {
  ...
  sockets: {
    connect() {
      ...
      userSetCode({ userId: this.$store.getters.userForm.userId });
    ...
    disconnect() {
      ...
      userOutCode({ userId: this.$store.getters.userForm.userId });
      this.$store.dispatch('communication/setUserStatus', 0);
    },
    //在线人数
    currentPersonList(res) {
      this.count = res;
      console.log('在线人数', res);
      this.$store.dispatch('communication/setUserStatus', this.count);
    },
    //群聊
    currentTextMsg(res) {
